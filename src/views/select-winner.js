import { IMAGE_SRC_URL, nextCaption, previousCaption } from "../api/http.js";
import { button, div, img, input, p } from "../lib/ui.js";

export const SelectWinner = ({ client, server }) => {
  const { didConfirm, isHost, name } = client;
  const { canVote, captions, index, uploader } = server;

  const caption = captions[index];
  if (isHost) {
    return div(img({ src: IMAGE_SRC_URL }), p(caption.top), p(caption.bottom));
  }
